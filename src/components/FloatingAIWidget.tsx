...
export default function FloatingAIWidget() {
  ...
  const statusDotCls = getStatusDot(step);
  const sub = getSubtitle(step, lastError);
            ...
            <div className="absolute right-0 top-14 w-72 rounded-2xl bg-white p-4 shadow-xl">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className={`text-[11px] ${sub.cls}`}>{sub.text}</div>
                </div>
              </div>
              <div className="mt-3">
                <ActionButton 
                  ...
                  retry={retry} 
                  hoverDisconnect={hoverDisconnect}
                  setHoverDisconnect={setHoverDisconnect}
                />
              </div>
            </div>
          )}
...
function getStatusDot(step: string) {
  switch (step) {
    case "CONNECTING":
...
function IconCircle({ step }: { step: string }) {
...
function getSubtitle(step: string, lastError: string | null) {
  switch (step) {
    case "CONNECTING":
...
interface ActionButtonProps {
  ...
  disconnect: () => void;
  retry: () => void;
  hoverDisconnect: boolean;
  setHoverDisconnect: (val: boolean) => void;
}

function ActionButton({ step, connect, disconnect, retry, hoverDisconnect, setHoverDisconnect }: ActionButtonProps) {
  if (step === "IDLE") {
    return (
      <button
        onClick={() => connect()}
